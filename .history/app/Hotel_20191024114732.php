    ...
    public function updateHotelInfo($request){
        ...
        if($auth_token == null){
            ...
            if($auth_row == null){
               ...
               dd(empty($checkHotelIdExist->items);
                if(empty($checkHotelIdExist->items)){
                    $res = array (
                        "type"=>'https://www.computerhope.com/jargon/u/unauacce.htm',
                        "message"=>'Hotel does not exists',
                        "detail"=>'The hotel you trying to update is either not exists or auth token is not correctt, Please check your request params again',
                        "error_code"=> 400,
                        "data"=>array()
                    ); 
                    return  Response::json([
