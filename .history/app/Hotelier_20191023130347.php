...
class Hotelier extends Model
{
    ...
    public function list($hotelier_id ,$noOfRecords,$start){
        
        $take = $noOfRecords == 0 ? Hotelier::get()->count(): $noOfRecords;
        $start = $start == null ? 0 : $start;
        $results = $this->leftjoin('locations', 'hotels.location', '=', 'locations.id')->where('hotelier_id',$hotelier_id)->where('status','1')->skip($start)->take($take)->get(); 
        if(count($results) > 0){
